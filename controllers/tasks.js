...
const TasksController = {
    ...
    async createTask(req, res) {
        try {
            const { projectId } = req.params;
            const { title, description, assignedTo, priority, estimatedHours, startDate, dueDate, tags } = req.body;
    ...
    async getTaskById(req, res) {
        try {
            const { id } = req.params;
            const task = await Task.findById(id).populate("assignedTo createdBy status");
